...
extern "C" DLLExport void init(Noise3D *noise) {
...
extern "C" DLLExport double lerp(double t, double a1, double a2) {
    return a1 + t * (a2 - a1);
}

extern "C" DLLExport double fade(double t) {
    return (((6 * t - 15) * t) + 10) * t * t * t;
}

extern "C" DLLExport double grad(int hash, double x, double y, double z) {
    int h = hash & 15;
    double u = h < 8 ? x : y;
...
extern "C" DLLExport double noise(Noise3D *noise, double x, double y, double z) {
    int X = (int)x & 255;
    int Y = (int)y & 255;
    int Z = (int)z & 255;
    x -= (int)x;
...
extern "C" DLLExport double fractalBrownianMotion(Noise3D *noise, double x, double y, double z, int octave) {
    double result = 0;
    double amplitude = 1;
